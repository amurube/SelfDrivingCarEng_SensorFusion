...
def pcl_to_bev(lidar_pcl, configs, vis=True):
    ...
    lidar_pcl_cp[:,0] = np.int_(lidar_pcl_cp[:,0] / bev_discret_x)

    # transform all metrix y-coordinates as well but center the foward-facing x-axis on the middle of the image
    lidar_pcl_cp[:,1] = np.int_(lidar_pcl_cp[:,1] / bev_discret_x + configs.bev_width/2)
    
    # shift level of ground plane to avoid flipping from 0 to 255 for neighboring pixels
    lidar_pcl_cp[:,2] = lidar_pcl_cp[:,2] - configs.lim_z[0]
    
    # re-arrange elements in lidar_pcl_cpy by sorting first by x, then y, then by decreasing height
    ind = np.lexsort((-lidar_pcl_cp[:,2], lidar_pcl_cp[:,1], lidar_pcl_cp[:,0]))
    lidar_pcl_height = lidar_pcl_cp[ind]

    # extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    _, idx_height_unique = np.unique(lidar_pcl_height[:, 0:2], axis = 0, return_index=True) # np.unique keeps the first point found which is the one with the highest intensity
    ...
    idx_intensity = np.lexsort((-lidar_pcl_cp[:, 3], lidar_pcl_cp[:, 1], lidar_pcl_cp[:, 0]))
    lidar_pcl_cpy = lidar_pcl_cp[idx_intensity]

    # only keep one point per grid cell
    _, indices = np.unique(lidar_pcl_cp[:, 0:2], axis=0, return_index=True)
    lidar_pcl_int = lidar_pcl_cp[indices]

    # create the intensity map. Normalization shall be done by the difference between the highest and lowest difference. This might change for other sensors
